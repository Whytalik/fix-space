...
**Тип тестування:** ручне, smoke-тест у браузері.

**Чеклист для кожної сторінки:**

- [ ] Сторінка відкривається без JS-помилок у консолі
- [ ] Дані завантажуються та відображаються коректно
- [ ] Форми надсилають запити (перевір у Network tab)
- [ ] Повідомлення про помилки відображаються коректно
- [ ] Сторінка доступна тільки авторизованим (перевір без токена)
...
### 6a. Автоматизоване тестування (Jest + React Testing Library)

**Що тестується:** рендер компонентів, props, події, класи.

**Тип тестування:** модульне, чорний ящик (тестуємо поведінку, не імплементацію).

**Структура тесту компонента:**
...
- [ ] Компонент рендериться з базовими props
- [ ] Порожні стани (no data, loading)
- [ ] Інтерактивність (кліки, зміни інпутів)
- [ ] Кастомні класи через `className`
