...
Цей документ пояснює **чому** обрано кожну технологію, а не **як** вона використовується. Детальна архітектура — у [architecture.md](./architecture.md), безпека — у [security.md](./security.md), фронтенд-стейт — у [frontend-state.md](./frontend-state.md).
...
**Модульна архітектура** — кожна фіча (Auth, Space, Record, Property тощо) є окремим NestJS-модулем із чіткими межами. Це безпосередньо відображається у структурі `apps/api/src/` (детально в [`architecture.md §5`](./architecture.md)).
...
**ACID-транзакції** — 4-pass ініціалізація простору (створення секцій → баз даних → властивостей → templates) виконується в одній транзакції (`prisma.$transaction()`). При помилці на будь-якому кроці — відбувається повний rollback.

**Зрілість і надійність** — PostgreSQL 16 підтримується хмарними платформами (Railway, Supabase, Google Cloud SQL). Деплой описаний у [`deployment.md`](../07-deployment/deployment.md).
...
Детальна конфігурація деплою — у [`deployment.md`](../07-deployment/deployment.md).
...
| ---------------------------------------------------------------- | ---------------------------------------------------------------- |
| [architecture.md](./architecture.md)                             | Загальна архітектура: модульна структура, API endpoints, патерни |
...
| [../07-deployment/deployment.md](../07-deployment/deployment.md) | Vercel + Railway, env vars, CORS, міграції                       |
