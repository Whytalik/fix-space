...
Цей документ пояснює **чому** обрано кожну технологію, а не **як** вона використовується. Детальна архітектура — у [`architecture.md`](./architecture.md), алгоритми — у [`algorithms.md`](./algorithms.md), безпека — у [`security.md`](./security.md), фронтенд-стейт — у [`frontend-state.md`](./frontend-state.md).
