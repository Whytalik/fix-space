  ...
  for (const record of records) {
    ...
    if (group.field === GroupField.CREATED_AT) {
      ...
      entry = isNaN(date.getTime())
        ? null
        : { key: dateSortKey(date, granularity), label: formatDateLabel(date, granularity, locale) };
      ...
      const pv = record.values?.find((v) => v.propertyId === group.propertyId);
      const type = getPropertyType(group.propertyId, properties);
      if (pv && type) {
        entry = extractGroupKey(pv.value, type, granularity, locale);
      }
    }
