...
const colWidthsKey = (id: string) => `db-col-widths:${id}`;

export function loadColumnWidths(id: string): Record<string, number> {
  try {
...
export function saveColumnWidths(id: string, widths: Record<string, number>): void {
  try {
    if (Object.keys(widths).length === 0) {
      localStorage.removeItem(colWidthsKey(id));
    } else {
      localStorage.setItem(colWidthsKey(id), JSON.stringify(widths));
    }
  } catch {
    // ignore localStorage errors
  }
