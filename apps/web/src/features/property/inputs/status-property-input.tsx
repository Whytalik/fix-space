...
export function StatusPropertyInput({ options, value, onChange, placeholder = "None", size = "md" }: StatusPropertyInputProps) {
  const className =
    size === "sm"
  ...
  function handleSelect(opt: StatusPropertyOption) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button type="button" onClick={() => setOpen((v) => !v)} onBlur={() => setTimeout(() => setOpen(false), 150)} className={className}>
        {value ? (
          <span
