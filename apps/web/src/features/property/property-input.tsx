...
export function PropertyInput({ property, value, onChange, relationRecordsMap }: PropertyInputProps) {
  ...
  switch (property.type) {
    ...
    case PropertyType.STATUS: {
      ...
      return <StatusPropertyInput options={allOptions} value={currentValue} onChange={onChange} placeholder={t("none")} />;
    }
    ...
    case PropertyType.RELATION: {
      ...
      return <RelationPropertyInput records={records} multiple={config?.multiple ?? false} value={value} onChange={onChange} />;
    }

    case PropertyType.PROGRESS:
      return <ProgressPropertyInput value={typeof value === "number" ? value : null} onChange={(val) => onChange(val)} />;
