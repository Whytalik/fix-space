    ...
    return (
      <div className="flex gap-3">
        {routing.locales.map((l) => (
          ...
          >
            <span className="text-sm">{l === "uk" ? "🇺🇦 Українська" : "🇺🇸 English"}</span>
          </button>
        ))}
      </div>
    );
          ...
          <div className="absolute right-0 mt-1 w-32 bg-elevated border border-stroke rounded-xl shadow-xl py-1 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            {routing.locales.map((l) => (
              <button
                key={l}
                onClick={() => switchLocale(l)}
                className={`w-full text-left px-3.5 py-2 text-[13px] font-semibold transition-colors hover:bg-surface ${
                  locale === l ? "text-accent bg-accent/5" : "text-ink-secondary hover:text-ink"
                }`}
              >
