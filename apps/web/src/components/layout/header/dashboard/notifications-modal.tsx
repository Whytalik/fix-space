...
import { AlertTriangle, Bell, CheckCheck, CircleAlert, Info, Trash2, X } from "lucide-react";
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-stroke">
          <div className="flex items-center gap-2">
            ...
            <span className="type-modal-title">{t("title")}</span>
            {hasUnread && (
              <span className="bg-accent text-white text-[11px] font-bold px-1.5 py-0.5 rounded-full">
            ...
            <div className="flex flex-col">
              {notifications.map((notification) => (
                <button
                  ...
                  className={`flex items-start gap-3 w-full px-4 py-3.5 text-left border-b border-stroke last:border-b-0 transition-colors duration-150 ${
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className={`text-sm ${notification.read ? "text-ink-secondary" : "text-ink font-semibold"}`}>
                        {notification.title}
                      </p>
                  ...
                  {!notification.read && <span className="shrink-0 w-2 h-2 rounded-full bg-accent mt-2 animate-pulse" />}
                </button>
              ))}
            </div>
          )}
