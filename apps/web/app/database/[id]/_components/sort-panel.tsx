    ...
    <div ref={setNodeRef} style={style}>
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          title={item.direction === SortDirection.ASC ? "Ascending" : "Descending"}
...
export function SortPanel({ onClose }: SortPanelProps) {
  ...
  const metaOptions: ComboboxOption[] = [
    { value: `meta:${SortField.CREATED_AT}`, label: "Created At" },
    { value: `meta:${SortField.UPDATED_AT}`, label: "Updated At" },
  ];
  ...
  return (
    <div
      ref={containerRef}
      className="absolute top-full right-0 mt-1 z-50 bg-elevated border border-stroke rounded-lg shadow-lg p-3 min-w-72 flex flex-col gap-2"
    >
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs font-medium text-ink-secondary uppercase tracking-wide">Sort</span>
      ...
      {sorts.length === 0 && !hasPendingRow && (
        <p className="text-xs text-ink-muted py-1">No sort rules. Add one below.</p>
      )}
