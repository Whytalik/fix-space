...
export function EditTemplatesSection({ database }: EditTemplatesSectionProps) {
  ...
  async function handleAddTemplate() {
    ...
    try {
      ...
      const created = await createTemplate({ databaseId: database.id, name: "Untitled template", isDefault: isFirst });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {templates.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-14 gap-3 text-center border border-dashed border-stroke rounded-xl">
          ...
          <p className="text-sm text-ink-muted max-w-xs">
            Templates let you pre-fill field values when creating new records.
          </p>
        </div>
      ) : (
      ...
      {deletingTemplate && (
        <ConfirmDialog
          title="Delete template"
          description={`"${deletingTemplate.name || "Untitled template"}" will be permanently deleted.`}
