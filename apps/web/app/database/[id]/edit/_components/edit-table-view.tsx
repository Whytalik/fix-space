...
function SortableTableRow({ row, onToggleVisibility }: { row: TableRow; onToggleVisibility: () => void }) {
  ...
  return (
    ...
    >
      ...
      {row.isPrimary && (
        <span className="text-[10px] font-semibold uppercase tracking-widest text-ink-muted bg-surface border border-stroke rounded px-1.5 py-0.5 mr-1">
          Primary
        </span>
      )}
      ...
      {!row.isPrimary && (
        <button
          ...
          title={row.prop.isVisible === false ? "Show column" : "Hide column"}
