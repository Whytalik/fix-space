...
describe('RecordService', () => {
  ...
  describe('create', () => {
    it('should create a record with property values and return RecordResponseDto', async () => {
      ...
      const mockPv1 = { id: 'pv-1', recordId: 'record-123', propertyId: 'prop-1', value: null, computed: false };
      const mockPv2 = { id: 'pv-2', recordId: 'record-123', propertyId: 'prop-2', value: null, computed: false };
      ...
      expect(prisma.property.findMany).toHaveBeenCalledWith({ where: { databaseId: 'db-123' } });
      ...
      expect(mockTx.propertyValue.create).toHaveBeenNthCalledWith(1, {
        data: { recordId: 'record-123', propertyId: 'prop-1', value: null, computed: false },
      });
      expect(mockTx.propertyValue.create).toHaveBeenNthCalledWith(2, {
        data: { recordId: 'record-123', propertyId: 'prop-2', value: null, computed: false },
      });
  ...
  describe('remove', () => {
    it('should delete record and return RecordResponseDto', async () => {
      ...
      expect(prisma.record.delete).toHaveBeenCalledWith({ where: { id: 'record-123' } });
