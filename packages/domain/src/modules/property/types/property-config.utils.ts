...
export function isTextPropertyConfig(config: unknown): config is TextPropertyConfig {
  return (
    ...
    ((config as Record<string, unknown>).defaultValue === null || typeof (config as Record<string, unknown>).defaultValue === "string") &&
    !("format" in config)
  );
}
...
export function isNumberPropertyConfig(config: unknown): config is NumberPropertyConfig {
  return (
    ...
    "format" in config &&
    "defaultValue" in config &&
    ((config as Record<string, unknown>).defaultValue === null || typeof (config as Record<string, unknown>).defaultValue === "number") &&
