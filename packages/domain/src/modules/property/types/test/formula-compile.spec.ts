...
describe("compileFormula", () => {
  ...
  describe("TC-FMLA-D-002: PLANNED_RR long", () => {
    it("produces (target - entry) / (entry - stop)", () => {
      ...
      const entryKey = key(ID_A);
      const targetKey = key(ID_B);
      const stopKey = key(ID_C);
      expect(result.expression).toBe(`(${targetKey} - ${entryKey}) / (${entryKey} - ${stopKey})`);
      expect(result.resultType).toBe(PropertyType.NUMBER);
    });
  });
  ...
  describe("TC-FMLA-D-003: PLANNED_RR short", () => {
    it("inverts formula for short positions", () => {
      ...
      const targetKey = key(ID_B);
      const stopKey = key(ID_C);
      expect(result.expression).toBe(`(${entryKey} - ${targetKey}) / (${stopKey} - ${entryKey})`);
    });
  });
  ...
  describe("TC-FMLA-D-009: CATEGORY_THRESHOLD", () => {
    it("nests IF chains with highest threshold outermost", () => {
      ...
      const f = key(ID_A);
      // Sorted descending: 80 > 50. Value 85 should return 'A', not 'B'.
      expect(result.expression).toBe(`IF(${f} > 80, 'A', IF(${f} > 50, 'B', 'C'))`);
      expect(result.resultType).toBe(PropertyType.TEXT);
    });

    it("returns elseLabel expression when thresholds are empty", () => {
      const result = compileFormula(FormulaPresetName.CATEGORY_THRESHOLD, {
        field: ID_A,
        thresholds: [],
        elseLabel: "None",
